...
公司設計系統的 Cursor 規則與 design tokens，供新 POC 或專案套用。本 repo **只包含 `.cursor`**。
...
1. Clone 本 repo，或下載 **`.cursor`**。
2. 將 **`.cursor`** 複製到你的專案根目錄（與 `package.json` 或專案根同層）。
3. 在 Cursor 開啟該專案，製作 POC 或公司產品時啟用規則 **「company-design-system」**。
4. 在專案中引入 `.cursor/company-design-system/all-variables.css`（路徑依專案調整）。
